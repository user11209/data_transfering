...
class DepthRenderer(nn.Module):
    ...
    def __init__(self, method: Literal["median", "expected", "dehaze"] = "median") -> None:
        super().__init__()
        self.method = method
    ...
    def forward(
        ...
        ray_indices: Optional[Int[Tensor, "num_samples"]] = None,
        num_rays: Optional[int] = None,
        density: Optional[Float[Tensor]] = None,
        ...
        if self.method == "dehaze":
            #DONE: change weight to dehaze weight, then do rendering as "expected" method
            dehaze_weight = (1- torch.exp(-torch.pow(density / thresh, p)))*weights
            eps = 1e-10
            steps = (ray_samples.frustums.starts + ray_samples.frustums.ends) / 2

            if ray_indices is not None and num_rays is not None:
                # Necessary for packed samples from volumetric ray sampler
                raise NotImplementedError("Dehaze depth calculation is not implemented for packed samples.")
            else:
                p = 4
                thresh = 1 / torch.min(ray_samples.frustums.ends - ray_samples.frustums.starts)
                depth = torch.sum(dehaze_weights * steps, dim=-2) / (torch.sum(weights, -2) + eps)
